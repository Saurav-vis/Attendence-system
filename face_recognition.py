...
class Face_Recognition:
    def __init__(self,root):
        ...
        img_top = Image.open(r"clg_img\face_detector1.jpg")
        img_top = img_top.resize((650,650),Image.ANTIALIAS)
        self.photoimg_top = ImageTk.PhotoImage((img_top))
        
        f_lbl = Label(self.root,image=self.photoimg_top)
        f_lbl.place(x=0,y=50,width=650,height=650)
        
        img_top1 = Image.open(r"clg_img\123.jpg")
        img_top1 = img_top1.resize((700,650),Image.ANTIALIAS)
        self.photoimg_top1 = ImageTk.PhotoImage((img_top1))
        
        f_lbl = Label(self.root,image=self.photoimg_top1)
        f_lbl.place(x=650,y=50,width=700,height=650)
        
        b1_1 = Button(f_lbl,text="Face Recognition",command=self.face_recog,cursor="hand2",font=("time new roman",18,"bold"),bg="green",fg="white")
        b1_1.place(x=250,y=575,width=200,height=40)
    ...
    def mark_attendance(self,i,r,n,d):
        with open("kiran.csv","r+",newline="\n") as f:
            ...
            if((i not in name_list) and (r not in name_list) and (n not in name_list) and (d not in name_list)):
                no=datetime.now()
                d1=no.strftime("%d/%m/%Y")
                dtString=no.strftime("%H:%M:%S")
    ...
    def face_recog(self):
        ...
        while True:
            ...
            cv2.imshow("Welcome to Face Reognition",img)
